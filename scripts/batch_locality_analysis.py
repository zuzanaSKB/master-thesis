...
if len(sys.argv) != 3:
    print("Usage: python3 batch_locality_analysis.py <script_path> <trace_file>")
    sys.exit(1)
...
plt.title("Locality analysis of the program num_gen_sort.py that sorts 1000 numbers - instruction addresses")
#plt.title("Locality analysis of the program fibonacci.py that computes F(1000) - data addresses")
plt.legend()
plt.grid(False)
plt.tight_layout()
plt.show()
...
plt.title("Address Alignment Statistics - num_gen_sort.py that sorts 1000 numbers (instruction addresses)")
#plt.title("Address Alignment Statistics - fibonacci.py that computes F(1000) (data addresses)")
plt.ylim(0, 100)
plt.grid(axis='y')
plt.tight_layout()
plt.show()
